...
ggsave("Figure S7.pdf", p, height = 100, width = 120, units = "mm", limitsize = FALSE)
...
p <- ggplot() +
  ...
  coord_quickmap()
print(p)
ggsave('./Figure 5 - map.pdf', p, height = 125, width = 250, units = "mm")
...
pts <- vect(data_cluster_map,
            ...
            crs  = "EPSG:4326")
bio_vals <- terra::extract(bio_10m, pts)   # 第一列是 ID
bio_vals <- bio_vals[,-1]                  # 去掉 ID 列
...
ggsave('./Figure 5c.png', p, height = 125, width = 250, units = "mm",   dpi = 1000)
...
p_future <- ggplot() +
  ...
  scale_x_continuous(breaks = c(-100, 0, 100))
print(p_future)
ggsave('./Figure 5 - future climate 2081-2100.png', p_future, height = 125, width = 250, units = "mm",   dpi = 1000)
